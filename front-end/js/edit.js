...
async function getUser() {
    ...
    <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" id="username" name="username" value="${user.data.username}">
            </div>
            <div class="form-group">
                <label for="place">Place:</label>
                <input type="text" id="place" name="place" value="${user.data.place}">
            </div>
            <div class="form-group">
                <label for="address">Address:</label>
                <textarea id="address" name="address" value="${user.data.address}">${user.data.address}</textarea>
            </div>
            <div class="form-group">
                <label for="phone">Phone:</label>
                <input type="tel" id="phone" name="phone" value="${user.data.phone}">
            </div>
            <div class="form-group">
                <label for="pincode">Pincode:</label>
                <input type="number" id="pincode" name="pincode" value="${user.data.pincode}">
            </div>
            <div class="form-group">
...
document.getElementById("frm").addEventListener("submit",async(e)=>{
    ...
    fetch(`http://localhost:3003/api/edituser/${id}`,{
        method:"PUT",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({username,email,place,profile,address,phone,pincode})
    }).then(async(res)=>{
        console.log(res);
